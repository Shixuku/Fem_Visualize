    ...
    : QWidget(parent)
{
	...
	connect(ui.pushButtonDis, &QPushButton::clicked, this, &Fem_Visualize::onShowDisplacement3);
...
void Fem_Visualize::onShowDisplacement2()
	{
		displacement->show();
		...
		for (auto& nodePair : m_structure->m_Nodes) {
			...
			for (int i = 0; i < 3; i++) { // 遍历X, Y, Z方向
				...
				else if (dofNum < m_structure->m_nOptimis)
				{
					// itotv 属于优化自由度组
					dofNum -= m_structure->m_nFixed; // 调整索引到 m_x2 的起点
					displacements[i] = m_structure->m_x2[dofNum];
				}
				else
				{
					displacements[i] = m_structure->m_x3[dofNum - m_structure->m_nOptimis];
				}
	...
	void Fem_Visualize::onShowDisplacement3()
	{
		displacement->show();
		...
		for (auto& nodePair : m_structure->m_Nodes) {
			...
			for (int i = 0; i < 3; i++) { // 遍历X, Y, Z方向
				displacements[i] = pNode->m_Disp[i];
